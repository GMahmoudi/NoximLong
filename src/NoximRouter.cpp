...
NoximRouter::NoximRouter(sc_module_name name): sc_module(name)
{
	SC_METHOD(rxProcess);
	sensitive << reset;
	sensitive << clock.pos();
				     ...
				     const NoximCoord & destination)
{
	...
	cout << "Herreeee: router at coordinations " <<  current.x << ", " << current.y << " uses the XY algorithm....." << endl;
...
{
cout <<"Herrrreeee, in NoximRouter::configure ....!" << endl;
  this->noc = noc;
	reservation_table.configure(noc,_id, DIRECTIONS);
    local_id = _id;
