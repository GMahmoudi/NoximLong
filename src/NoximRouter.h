...
SC_MODULE(NoximRouter)
{
   ...
   // Constructor	
   SC_HAS_PROCESS(NoximRouter);
   NoximRouter (sc_module_name name);

//Still, the constructor could be written as usual..
 /*   SC_CTOR(NoximRouter) {
	SC_METHOD(rxProcess);
	sensitive << reset;
	sensitive << clock.pos();
...
*/
  ...
  private:
    ...
    vector < int >routingFullyAdaptive(const NoximCoord & current,
				       const NoximCoord & destination);
    vector < int >routingFullyAdaptive_5(const NoximCoord & current,
				       const NoximCoord & destination);
