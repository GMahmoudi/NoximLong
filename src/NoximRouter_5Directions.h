...
#ifndef __NOXIMROUTER_5Directions_H__
#define __NOXIMROUTER_5Directions_H__
...
SC_MODULE(NoximRouter_5Directions)
{
    ...
    sc_in <NoximFlit> flit_rx[DIRECTIONS_FIVE + 1];	  // The input channels (including local one)
    sc_in <bool> req_rx[DIRECTIONS_FIVE + 1];	  // The requests associated with the input channels
    sc_out <bool> ack_rx[DIRECTIONS_FIVE + 1];	  // The outgoing ack signals associated with the input channels

    sc_out <NoximFlit> flit_tx[DIRECTIONS_FIVE + 1];   // The output channels (including local one)
    sc_out <bool> req_tx[DIRECTIONS_FIVE + 1];	  // The requests associated with the output channels
    sc_in <bool> ack_tx[DIRECTIONS_FIVE + 1];	  // The outgoing ack signals associated with the output channels

    sc_out <int> free_slots[DIRECTIONS_FIVE + 1];
    sc_in <int> free_slots_neighbor[DIRECTIONS_FIVE + 1];
    ...
    int selection_type;
    NoximBuffer buffer[DIRECTIONS_FIVE + 1];	        // Buffer for each input channel 
    bool current_level_rx[DIRECTIONS_FIVE + 1];	// Current level for Alternating Bit Protocol (ABP)
    bool current_level_tx[DIRECTIONS_FIVE + 1];	// Current level for Alternating Bit Protocol (ABP)
    ...
    SC_CTOR(NoximRouter_5Directions) {
  ...
  private:
    ...
    vector < int >routingXY_naiv5(const NoximCoord & current,
			      const NoximCoord & destination);
    vector < int >routingXY_simple5(const NoximCoord & current,
			      const NoximCoord & destination);
