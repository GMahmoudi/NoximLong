...
void NoximRouter_5Directions::rxProcess()
{
    if (reset.read()) {
	// Clear outputs and indexes of receiving protocol
	for (int i = 0; i < DIRECTIONS_FIVE + 1; i++) {
	    ack_rx[i].write(0);
...
void NoximRouter_5Directions::txProcess()
{
  if (reset.read()) 
    {
  ...
  else 
    {
      ...
      for (int j = 0; j < DIRECTIONS_FIVE + 1; j++) 
	{
	  int i = (start_from_port + j) % (DIRECTIONS_FIVE + 1);
	      ...
	      if (o != NOT_RESERVED) 
		{
		  if (current_level_tx[o] == ack_tx[o].read()) 
			...
			reservation_table.release(o);
		      ...
		      // Update stats
		      if (o == DIRECTION_LOCAL_For5DirectionsRouters) 
			{
		      ...
		      else if (i != DIRECTION_LOCAL_For5DirectionsRouters) 
			{
...
NoximNoP_data NoximRouter_5Directions::getCurrentNoPData() const
{
    NoximNoP_data NoP_data;

    for (int j = 0; j < DIRECTIONS_FIVE; j++) {
...
void NoximRouter_5Directions::bufferMonitor()
{
    if (reset.read()) {
	for (int i = 0; i < DIRECTIONS_FIVE + 1; i++)
...
vector <
    int >NoximRouter_5Directions::routingFunction(const NoximRouteData & route_data)
{
    ...
    case ROUTING_XY_naiv5:
	return routingXY_naiv5(position, dst_coord);

    case ROUTING_XY_simple5:
	return routingXY_simple5(position, dst_coord);
...
int NoximRouter_5Directions::route(const NoximRouteData & route_data)
{
    stats.power.Routing();

    if (route_data.dst_id == local_id)
	return DIRECTION_LOCAL_For5DirectionsRouters;
...
void NoximRouter_5Directions::NoP_report() const
{
...
int NoximRouter_5Directions::NoPScore(const NoximNoP_data & nop_data,
...
int NoximRouter_5Directions::selectionNoP(const vector < int >&directions,
...
int NoximRouter_5Directions::selectionBufferLevel(const vector < int >&directions)
{
...
int NoximRouter_5Directions::selectionRandom(const vector < int >&directions)
{
    return directions[rand() % directions.size()];
}

int NoximRouter_5Directions::selectionFunction(const vector < int >&directions,
				   const NoximRouteData & route_data)
{
...
vector < int >NoximRouter_5Directions::routingXY(const NoximCoord & current,
...
vector < int >NoximRouter_5Directions::routingXY_naiv5(const NoximCoord & current, const NoximCoord & destination)
{
    vector < int >directions;
	cout << "Herreeee: router at coordinations " <<  current.x << ", " << current.y << " uses the XY_naiv5 algorithm..............."<<endl;
	int pairNodeID = noc->getIDofPairNode(noc->XY2Id(current.x,current.y));
	if(pairNodeID == noc->XY2Id(destination.x,destination.y))
	{//route over the long connection.
	 cout << "routing over the long connection, from node : " << noc->XY2Id(current.x,current.y) << " to node: " << pairNodeID << endl;
		 directions.push_back(DIRECTION_LongConnection);
        }
	else return routingXY(current, destination);
	return directions;
}

vector < int >NoximRouter_5Directions::routingXY_simple5(const NoximCoord & current, const NoximCoord & destination)
{
    cout << "Herreeee: router at coordinations " <<  current.x << ", " << current.y << " uses the XY_simple5 algorithm..............."<<endl;
    vector < int >directions;
    int hopCount_xy = abs(current.x - destination.x) + abs(current.y - destination.y);
    int pairNodeID = noc->getIDofPairNode(noc->XY2Id(current.x,current.y));
    NoximCoord pairNodeCoord = id2Coord( pairNodeID);

    int hpCout_LongConn = 1 + abs(destination.x - pairNodeCoord.x) + abs(destination.y - pairNodeCoord.y);
    if(hopCount_xy > hpCout_LongConn)
    {
	//route over the long connection.
	cout << "routing over the long connection, from node : " << noc->XY2Id(current.x,current.y) << " to node: " << pairNodeID << endl;
	directions.push_back(DIRECTION_LongConnection);
...
vector < int >NoximRouter_5Directions::routingWestFirst(const NoximCoord & current,
					    const NoximCoord & destination)
{
    vector < int >directions;
...
vector < int >NoximRouter_5Directions::routingNorthLast(const NoximCoord & current,
					    const NoximCoord & destination)
{
    vector < int >directions;
...
vector < int >NoximRouter_5Directions::routingNegativeFirst(const NoximCoord & current,
						const NoximCoord &
						destination)
{
    vector < int >directions;
...
vector < int >NoximRouter_5Directions::routingOddEven(const NoximCoord & current,
...
vector < int >NoximRouter_5Directions::routingDyAD(const NoximCoord & current,
				       const NoximCoord & source,
				       const NoximCoord & destination)
{
    vector < int >directions;
...
vector < int >NoximRouter_5Directions::routingFullyAdaptive(const NoximCoord & current,
						const NoximCoord &
						destination)
{
    vector < int >directions;
...
vector < int >NoximRouter_5Directions::routingTableBased(const int dir_in,
...
void NoximRouter_5Directions::configure(const int _id,
			    const double _warm_up_time,
			    const unsigned int _max_buffer_size,
			    NoximGlobalRoutingTable & grt, NoximNoC *noc)
{
 this->noc = noc;
	reservation_table.configure(noc, _id, DIRECTIONS_FIVE);
    local_id = _id;
    stats.configure(_id, _warm_up_time);

    start_from_port = DIRECTION_LOCAL_For5DirectionsRouters;
...
unsigned long NoximRouter_5Directions::getRoutedFlits()
{
    return routed_flits;
}

unsigned int NoximRouter_5Directions::getFlitsCount()
{
    unsigned count = 0;
...
double NoximRouter_5Directions::getPower()
{
    return stats.power.getPower();
}

int NoximRouter_5Directions::reflexDirection(int direction) const
{
...
int NoximRouter_5Directions::getNeighborId(int _id, int direction) const
{
...
bool NoximRouter_5Directions::inCongestion()
{
    for (int i = 0; i < DIRECTIONS; i++) {
	int flits =
...
void NoximRouter_5Directions::ShowBuffersStats(std::ostream & out)
{
  for (int i=0; i<DIRECTIONS+1; i++)
